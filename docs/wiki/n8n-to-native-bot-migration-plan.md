...
**Deployed live same session**: committed (`6eb21db`) on `native-telegram-bot`. VPS `78.17.160.248` `.env` updated to `TELEGRAM_BOT_MAX_POLL_ATTEMPTS=120`; `backend/app/config.py` copied directly to `/opt/autosnab_mvp/backend/app/config.py` (single-file deploy, not a full `git archive` sync, to avoid any risk of the repo-tracked `autosnab_mvp.db`/`exports/*.csv` overwriting live state — those live in Docker-managed named volumes anyway, not host bind mounts, but minimal blast radius was preferred). `docker compose --profile public-ip build backend` + `up -d --no-deps backend` recreated the container; verified `healthy`, zero errors in logs, and `docker exec ... python -c "from app.config import settings; ..."` inside the running container confirms `max_poll_attempts=120`. One live HTTPS connection to the Telegram Bot API confirms the poller is still running. Disk usage after rebuild: 68% → 82% (2.6G free) — same recurring build-cache growth pattern noted on 2026-07-20/2026-07-21; not urgent but still not automated.
