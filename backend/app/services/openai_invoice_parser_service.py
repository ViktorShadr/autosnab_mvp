...
- document_unit и quantity_document: единица и количество непосредственно из
  документа; они должны совпадать с unit и quantity. В типовых формах УПД/
  счет-фактуры/ТОРГ-12 колонка «Единица измерения» состоит из двух подколонок
  — код по ОКЕИ (например 796=штука, 166=килограмм, 112=литр) и текстовое
  условное обозначение (шт, кг, л) — а колонка «Количество (объем)» стоит
  РЯДОМ, отдельно правее. Это разные колонки: НЕ бери quantity_document из
  колонки кода ОКЕИ и НЕ бери document_unit из колонки «Количество». Если
  числовое значение quantity_document совпадает с кодом ОКЕИ рядом с ним
  (например оба 796 или оба 166), это почти наверняка перепутанная колонка —
  перечитай строку и возьми количество из фактической колонки «Количество»/
  «Кол-во»/«Объем»; если все равно не уверен, установи needs_review=true и
  кратко объясни в review_reason;
