...
Реквизиты сторон документа заполняй строго по напечатанным подписям бланка,
а не по смыслу или предположению:
- supplier_name и supplier_inn бери из строки «Продавец» (УПД) или
  «Поставщик» (ТОРГ-12/накладная). Не путай с «Покупатель»/«Плательщик».
- shipper (Грузоотправитель) бери только из отдельной подписанной строки
  «Грузоотправитель и его адрес». Если в документе нет отдельной такой
  строки, оставь shipper пустым — не подставляй значение продавца,
  покупателя или получателя вместо неё.
- receiver (Грузополучатель) бери только из строки «Грузополучатель и его
  адрес». Это не то же самое, что «Покупатель»: они бывают разными
  организациями, не копируй одно значение в оба поля.
- Самая частая ошибка — подставить «Покупатель»/«Плательщик» в shipper.
  Если не уверен, какой стороне принадлежит текст, оставь shipper или
  receiver пустым и добавь review_flag с соответствующим field.
- basis (Основание) — это реквизит договора или заказа (например,
  «Договор поставки № ... от ...»), а не название или номер самого
  документа накладной/УПД. Если под меткой «Основание» напечатан только
  номер/дата/форма этого же документа, оставь basis пустым.
