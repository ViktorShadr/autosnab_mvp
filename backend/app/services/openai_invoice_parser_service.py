...
- package: фасовка, объем, вес или размер одной базовой единицы товара с
  исходным написанием в package.raw;
- package.dry_weight и package.dry_weight_unit: заполняй только когда в
  документе отдельно указан сухой/отжатый/чистый вес без рассола или заливки
  (например «300г/150г отжатый вес» для оливок и маслин). Если указан только
  один вес (брутто), оставляй dry_weight null — не вычисляй его сам;
...
фасовки multiplier=1. Для 0,5Л 12ШТ multiplier=6 и единица учета л. Если в отдельном столбце
«Состав ед. изм.» указано 6, а quantity_document=2 УП и package=1,5Л, верни
units_per_package=6; итоговый коэффициент backend рассчитает как 6 * 1,5 = 9 л
на одну УП, а итоговое количество в УС как 2 * 9 = 18 л.
...
- «МАСЛИНЫ Б/К 300Г/150Г ОТЖ.ВЕС» -> package.value 300, package.unit «г»,
  package.dry_weight 150, package.dry_weight_unit «г».

Все поля нормализации являются кандидатами: окончательное сопоставление с
листами «Товары» и «Справочник фасовок», выбор способа пересчета количества
(без пересчета, по количеству вложений, по весу/объему, по сухому весу, ручная
проверка), расчеты, статусы и запись в Google Sheets выполняет backend по
таблице правил. Backend применяет пересчет только когда правило это явно
подтверждает; твои multiplier/accounting_unit_candidate — не более чем
подсказка. Ты не выбираешь колонки и не пишешь во внешние системы."""
