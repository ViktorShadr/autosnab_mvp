...
_DOCUMENT_FORM_BY_PROVIDER = {
    "diadoc": "УПД/ЭДО Диадок",
    "sbis": "УПД/ЭДО СБИС",
}
_REQUEST_ID_PREFIX_BY_PROVIDER = {
    "diadoc": "DIADOC",
    "sbis": "SBIS",
}


def parse_fns_invoice_xml(
    content: bytes,
    *,
    file_id: str,
    ...
    """Parses a УПД/счёт-фактура XML in the ФНС government schema.

    The tag names below (СвСчФакт, СвПрод, ТаблСчФакт, СведТов, ...) are the
    ФНС УПД/счёт-фактура standard, not specific to any one EDO operator —
    this function is shared across providers that deliver formalized XML in
    that schema (Diadoc, SBIS). Only the three provider-literal fields below
    (document_form, parser_metadata, request_id prefix) vary by provider.
    """
    ...
    request_id_prefix = _REQUEST_ID_PREFIX_BY_PROVIDER.get(provider, provider.upper())
    document_form = _DOCUMENT_FORM_BY_PROVIDER.get(provider, f"УПД/ЭДО {provider}")
    ...
    return InvoiceReviewCreateRequest(
        ...
        request_id=f"{request_id_prefix}-{file_id}",
        supplier=supplier,
