...
from app.services.bot_gateway_service import (
    append_draft_page,
    finalize_draft,
    get_draft_status,
    get_latest_upload_status,
    get_upload_status,
    reset_draft,
...
def get_bot_draft_status(chat_id: str = Query(...), db: Session = Depends(get_db)):
    return get_draft_status(db, chat_id)
...
def get_latest_bot_upload_status(chat_id: str = Query(...), db: Session = Depends(get_db)):
    status = get_latest_upload_status(db, chat_id)
    if status is None:
...
def get_bot_upload_status(upload_id: str, db: Session = Depends(get_db)):
    try:
        return get_upload_status(db, upload_id)
    except ValueError as exc:
