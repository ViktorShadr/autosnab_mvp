...
В таблице сейчас 39 колонок. Актуальная структура заголовков:
...
Важно: отдельных столбцов `ЕГАИС`, `Меркурий`, `Честный знак` больше нет. Вместо них используется один общий столбец `Госсистемы`. Столбец `Статус строки` находится в начале таблицы.

Столбец `Ссылка на исходный документ` оставлен в шапке, но системой автоматически не заполняется.

Оформление листа `Накладные` применяется автоматически: первая строка закреплена и окрашена в темно-зеленый цвет, строки таблицы идут с чередованием белого и светло-серого фона. Конец каждой загруженной накладной отделяется горизонтальной линией по последней строке документа.
